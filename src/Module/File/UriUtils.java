 ...
 *
 * <p>All {@code encode*(String, String} methods in this class operate in a
 * similar way: <ul> <li>Valid characters for the specific URI component as
 * defined in RFC 3986 stay the same. <li>All other characters are converted
 * into one or more bytes in the given encoding scheme. Each of the resulting
 * bytes is written as a hexadecimal string in the "<code>%<i>xy</i></code>"
 * format. </ul>
 *
...
public abstract class UriUtils {
    ...
    private static final Pattern URI_PATTERN = Pattern.compile("^(" + SCHEME_PATTERN + ")?" + "(//(" + USERINFO_PATTERN
            + "@)?" + HOST_PATTERN + "(:" + PORT_PATTERN + ")?" + ")?" + PATH_PATTERN + "(\\?" + QUERY_PATTERN + ")?"
            + "(#" + LAST_PATTERN + ")?");
     ...
     * components are encoded according to their respective valid character
     * sets. <p><strong>Note</strong> that this method does not support
     * fragments ({@code #}), as these are not supposed to be sent to the
     ...
     *             when the given encoding parameter is not supported
     */
    public static  String encodeUriComponents(String scheme, String authority, String userinfo, String host,
     ...
     * following rules: <ul> <li>Alphanumeric characters {@code "a"} through
     * {@code "z"}, {@code "A"} through {@code "Z"}, and {@code "0"} through
     * {@code "9"} stay the same. <li>Special characters {@code "-"},
     * {@code "_"}, {@code "."}, and {@code "*"} stay the same. <li>All other
     * characters are converted into one or more bytes using the given encoding
     * scheme. Each of the resulting bytes is written as a hexadecimal string in
     * the {@code %xy} format. <li>A sequence "<code>%<i>xy</i></code>" is
     * interpreted as a hexadecimal representation of the character. </ul>
     *
     * @param source
     *            the source string
     * @param encoding
